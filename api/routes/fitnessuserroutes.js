...
const fitnessusermodel = require("../models/fitnessusermodel");

const router = express.Router();


router.get("/_user",async(req,res)=>{
    const data= await fitnessusermodel.find({})
...
router.post("/create_user",async(req,res)=>{
    const data=new fitnessusermodel(req.body)
    await data.save()
...
router.put("/update_user",async(req,res)=>{
    ...
    const data=await fitnessusermodel.updateOne({_id:id},rest)
...
const data=await fitnessusermodel.deleteOne({_id:id})
...
router.get("/count_user",async(req,res)=>{
    try{
        const users=await fitnessusermodel.find({});
...
router.get("/order_user/:id", async (req, res) => {
    ...
    try {
        const order = await fitnessusermodel.findById(id);
...
router.post("/login_user", async (req, res) => {
    ...
    try {
        console.log(email);  
      const user = await fitnessusermodel.findOne({ email });
