...
function UpdateAccount(){
    ...
    useEffect(() => {
        const fetchUserData = async () => {
          try {
            const response = await fetch(`http://localhost:3500/compaintuser/order_customer/${id}`);
      ...
      const handleUpdate = async () => {
        try {
          const response = await fetch(`http://localhost:3500/compaintuser/update_customer`, {
            method: 'PUT',
            headers: {
