...
const getfetchdata=async()=>{
    try{
    const data=await axios.get("http://localhost:3500/fitnessuser/_user")
...
const handledelete= async(id)=>{
    const data=await axios.delete("http://localhost:3500/fitnessuser/delete_user/"+id)
    if(data.data.success){
        getfetchdata()
                     ...
                     showdiscounts.map((e1)=>{
                      return(
                          <tr> 
                            ...
                            <td>
                              <a href={`/fitness/updateuser/${e1._id}`}>Edit Details</a>
